 ...
 * ------------------------ */
   ...
   */
   ...
   public double[] getAverageVector(){
      double[] vectorToReturn = new double[m];
      for(int i = 0; i < n ; i++){
         double sum = 0;
         for(int j = 0; j< m ; j++){
            sum += Math.abs(A[i][j]);
         }
         vectorToReturn[i] = sum / m;
      }
      return vectorToReturn;
   }

   public double[][] normalize1Matrix () {
      double[][] returnMatrix = new double[m][n];
      for (int j = 0; j < n; j++) {
         double s = 0;
         for (int i = 0; i < m; i++) {
            returnMatrix[i][j] = Math.abs(A[i][j]);
            s += Math.abs(A[i][j]);
         }
         for(int i = 0; i < m ; i++){
            returnMatrix[i][j] = returnMatrix[i][j] / s;
         }
      }
      return returnMatrix;
   }
